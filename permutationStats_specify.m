function [p,greater] = permutationStats_specify(data,column,conds,stats_params,project_name)
...
if stats_params.all_trials
    all_trials = cell(size(data.trialinfo,1),1);
    for i  = 1:size(data.trialinfo,1)
        all_trials{i} = 'all_trials';
    end
    data.trialinfo.all_trials = all_trials;
else
end
...
[grouped_trials,~] = groupConds(conds,data.trialinfo,column,stats_params.noise_method,stats_params.noise_fields_trials,false);
    ...
    if strcmp(project_name,'VTCLoc')
         rng('default')
         grouped_trials{1} = grouped_trials{1};
         grouped_trials{2} = vertcat(grouped_trials{2:end});
         grouped_trials{2} = randsample(grouped_trials{2},size(grouped_trials{1},1));
    else
    end
