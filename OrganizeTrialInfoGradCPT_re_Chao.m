...
%     dprime_overall=ans(34);
    ...
    elseif Windows=='1'
        CO_indices=find(response(:,1)==2 & response(:,2)==0);%chao changed here
        CE_indices=find(response(:,1)==2 & response(:,2)~=0);%chao changed here
        CC_indices=find(response(:,1)==1 & response(:,2)~=0);%chao changed here
        OE_indices=find(response(:,1)==1 & response(:,2)==0);
    end
    ...
    CEs_total=length(CE_indices);%? what is for  chao
    COs_total=length(CO_indices);%? waht is for  chao
    
    total_time=endtime-starttime;%? waht is for  chao
%     ECoG_start=TTL_onsets(2);
    
    % Get CE RTsF
    CE_RTs=response(CE_indices,5);%? waht is for  chao
